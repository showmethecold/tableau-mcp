  ...
  **Example usage**
  ...
  - Also include each step's column schema (verbose):
    ...
    callback: async ({ flowId, includeFieldSchemas }, extra): Promise<CallToolResult> => {
      // describe-flow only ever needs to (1) resolve the flow's identity and
      // (2) download its sanitized document. Keep the JWT mint to exactly those
      // scopes so the call succeeds against connected apps that grant only the
      // flow scopes (mirrors get-flow's per-call scope narrowing).
