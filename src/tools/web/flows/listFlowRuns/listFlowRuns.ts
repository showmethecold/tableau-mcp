      ...
      return await listFlowRunsTool.logAndExecute<ListFlowRunsResult>({
        ...
        callback: async () => {
          return new Ok(
            await useRestApi({
              ...extra,
              jwtScopes: listFlowRunsTool.requiredApiScopes,
              callback: async (restApi) => {
                ...
                const failureInsight = await buildFailureInsight({ flowRuns, restApi });
...
async function buildFailureInsight({
  ...
  if (exampleFlowId !== undefined) {
    try {
      const { flow } = await restApi.flowsMethods.queryFlow({
        siteId: restApi.siteId,
