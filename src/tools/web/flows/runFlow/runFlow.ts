...
const paramsSchema = {
  ...
  outputStepIds: z.array(z.string().nonempty()).optional(),
...
export const getRunFlowTool = (server: WebMcpServer): WebTool<typeof paramsSchema> => {
  ...
  **Parameters:**
  ...
  - \`outputStepIds\` (optional) – Run only these output steps (ids from \`get-flow\`). Omit to run every output step.
  ...
  **Requirements & limits:**
  ...
  - Requires Tableau REST API access scopes \`tableau:flows:run\`, \`tableau:flows:read\` (for bounded-context verification), and \`tableau:mcp_site_settings:read\`.`,
    ...
    annotations: {
      title: 'Run Flow',
      readOnlyHint: false,
      // A run does not delete/alter the flow definition or its schedule; it
      // enqueues an execution. So not "destructive", but explicitly not
      // idempotent — each call is another run.
    ...
    callback: async (
      ...
      return await runFlowTool.logAndExecute<RunFlowResult>({
        extra,
        args: { flowId, runMode, outputStepIds, parameterOverrides },
        callback: async () => {
